...
- **自更新安装包下载完成后可直接操作**：完成提示条上新增「打开位置」（资源管理器定位安装包）与「立即安装」（直接运行安装包）两个动作按钮
- **提示信息统一 10 秒滞留**：所有 toast 提示滞留时间统一为 10 秒（原 4.2 秒），且均支持点击 ✕ 手动关闭

### v0.3.2

- **修复**：已下载的新版 `.vsix` 不被识别（如 `0.27.2026082200` 型长版本号）——扫描排序误用字符串比较，`0.27.6` 字符串序大于 `0.27.2026082200` 导致旧文件一直占据列表；现前后端均改为版本号语义化比较，同插件始终保留最高版本
